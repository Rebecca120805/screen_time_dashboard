...
from data_loader import list_week_files, load_week_data, DATA_DIR
from data_analyzer import get_top_5_apps_per_day, get_daily_total
...
selected_week = st.selectbox("Wähle eine Kalenderwoche", week_files)
...
try:
    ...
    most_used_app = df.groupby("app")["usage_minutes"].sum().idxmax()
...
col1.metric(label="⏱️ Ø Nutzung / Tag", value=f"{avg_daily_time} min")

col2.metric(label="📱 Gesamtzeit Woche", value=f"{total_week_time} min")

col3.metric(label="📅 Erfasste Tage", value=str(num_days))

col4.metric(label="🏆 Höchster Nutzungstag", value=f"{max_day} ({max_day_time} min)")

col5.metric(label="⭐ Top App der Woche", value=most_used_app)
...
try:
    ...
    logger.critical(
        "Kritischer Fehler bei der Berechnung der WOchennutzung", exc_info=True
    )
...
comparison_df["percentage"] = (
    comparison_df["screen_time"] / comparison_df["uni_time"]
) * 100
...
selected_date = st.selectbox("Wähle einen Tag für die Detailanalyse", available_dates)
